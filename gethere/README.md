...
Le frontend définit les titres, descriptions, URLs canoniques, balises Open Graph et données structurées `Product` côté client. Avant le déploiement, copier `.env.example` vers `.env` et renseigner `VITE_SITE_URL` avec l'URL publique finale, sans slash final.

Le serveur d'hébergement doit rediriger les routes React vers `index.html` pour que `/boutique`, `/categories` et `/produit/:id` restent accessibles après un rechargement. Publier ensuite `https://votre-domaine.com/robots.txt` et `https://votre-domaine.com/sitemap.xml`, puis déclarer le sitemap dans Google Search Console et Bing Webmaster Tools.

Le sitemap statique contient les pages éditoriales. Les fiches produits étant chargées depuis l'API, elles devront être ajoutées au sitemap généré côté serveur dès que le domaine et la source de vérité des produits sont stabilisés.
